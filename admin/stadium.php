...
$get_data = "select * from piyush_stadium where status=1";
$stadium_data = mysqli_query($conn, $get_data);
...
if (isset($_REQUEST['submit'])) {
    ...
    if ($upload) {
        ...
        $qry = "insert into piyush_stadium(gid,name,image) values('$gid','$name','$file')";
        $result = mysqli_query($conn, $qry);

        if ($result) {

            echo "<script>alert('Data Inserted Successfuly!!')</script>";
            echo "<script>window.location='stadium.php'</script>";
        } else {
...
if (isset($_GET['edit_id'])) {
    ...
    $id = $_GET['edit_id'];
    $edit_qry = "select * from piyush_stadium where id='$id'";
    $update_qry = mysqli_query($conn, $edit_qry);
    $view_details = mysqli_fetch_array($update_qry);
}
...
if (isset($_REQUEST['update'])) {
    ...
    if ($upload) {
        ...
        $qry = "update piyush_stadium set gid='$gid', name='$name',image='$file' where id='$id'";
        $result = mysqli_query($conn, $qry);
...
if (isset($_GET['del_id'])) {
    ...
    $qry = "update piyush_stadium set status=2 where id='$id'";
    $result = mysqli_query($conn, $qry);
...
<body>
    ...
    <div class="container-fluid">
        <div class="container ">
            <h2 class="fs-5 pt-4 pb-2 text-center">Insert a new Image or Video</h2>
            <div class="row justify-content-center">
                <div class="col-12 col-lg-6 col-md-12 col-sm-12">
                    <form method="post" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="name" class="form-label">Name</label>
                            <input type="text" value="<?= @$view_details['name']; ?>" class="form-control" name="name">
                        </div>
    ...
    <div class="container-fluid mt-3 -mb-2">
        <div class="container">
            <h2 class="text-center pt-5">View Table Data</h2>
            <div class="row  mt-4 mb-3">
                <div class="col-12 col-lg-12 col-md-12 col-sm-12">
                    <table class="table" id="myTable">
                        ...
                        <tbody>
                            <?php $i = 1;
                            foreach ($stadium_data as $Data) { ?>
                                <tr>
                                    <td><?php echo $i; ?></td>
                                    <td><?php echo $Data['name']; ?></td>
                                    <td>
                                        <?php if ($Data['image']) { ?>
                                            <img src="../assets/uploads/<?php echo $Data['image']; ?>" width="300px"
                                                height="200px" />
                                        <?php } else { ?>
                                        <?php } ?>
                                    </td>
